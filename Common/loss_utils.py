...
from Common.pointconv_util import kernel_density_estimation_ball
...
def density_loss(pcd1, pcd2):
    density1 = kernel_density_estimation_ball(pcd1, radius=0.1, sigma=0.1)
    density2 = kernel_density_estimation_ball(pcd2, radius=0.1, sigma=0.1)
    loss = tf.reduce_mean(density1 - density2)
    return loss
...
def chamfer(pcd1, pcd2, radius=1.0):
    ...
    # print('dists_forward', dists_forward.shape)
    # print('dists_backward', dists_backward.shape)
...
def get_uniform_loss3(pcd, percentages=[0.004, 0.006, 0.008, 0.010, 0.012], radius=1.0):
    B, N, C = pcd.get_shape().as_list()
    npoint = int(N * 0.05)
    loss = []
    for p in percentages:
        ...
        grouped_pcd = group_point(pcd, idx)     # b, npoint, nsample, 3
        grouped_pcd = tf.concat(tf.unstack(grouped_pcd, axis=1), axis=0)        # b*npoint, nsample, 3
        grouped_pcd_2 = gather_point(grouped_pcd, farthest_point_sample(2, grouped_pcd))        # b*npoint, 2, 3
        uniform_dis = tf.reduce_mean(grouped_pcd_2[:, 0, :]**2 - grouped_pcd_2[:, 1, :]**2)       # b*npoint, 3
        # uniform_dis = tf.reduce_mean(uniform_dis)        # 1
        loss.append(uniform_dis)
...
def get_repulsion_loss(pred, nsample=20, radius=0.07, knn=False, use_l1=False, h=0.001):
    ...
    if knn:
        ...
        # pts_cnt = tf.constant(nsample, shape=(24, 20))
    else:
...
def get_norm_loss(pred, nsample=5, radius=0.07, knn=False, use_l1=False, h=0.001):
    pc, norm = pred[..., :3], pred[..., 3:]
    if knn:
        _, idx = knn_point(nsample, pc, pc)
        # pts_cnt = tf.constant(nsample, shape=(24, 20))
    else:
        idx, pts_cnt = query_ball_point(radius, nsample, pc, pc)
    # tf.summary.histogram('smooth/unque_index', pts_cnt)

    grouped_norm = group_point(norm, idx)  # (batch_size, npoint, nsample, 3)
    grouped_norm -= tf.expand_dims(norm, 2)

    # get the uniform loss
    if use_l1:
        dists = tf.reduce_sum(tf.abs(grouped_norm), axis=-1)
    else:
        dists = tf.reduce_sum(grouped_norm ** 2, axis=-1)
    ...
    if use_l1:
        h = np.sqrt(h)*2
    print(("norm_loss h is ", h))

    val = tf.maximum(0.0, h + val)  # dd/np.sqrt(n)
    norm_loss = tf.reduce_mean(val)
    return norm_loss


def get_norm_loss2(pred, gt):
    return
