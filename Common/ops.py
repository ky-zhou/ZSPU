...
def up_block(inputs, up_ratio, scope='up_block', is_training=True, bn_decay=None):
    with tf.variable_scope(scope,reuse=tf.AUTO_REUSE):
        ...
        print('up_block net2:', net.shape)

        net = attention_unit(net, is_training=is_training)
        print('up_block net3:', net.shape)

        net = conv2d(net, 256, [1, 1], padding='VALID', stride=[1, 1],
                     bn=False, is_training=is_training, scope='conv1', bn_decay=bn_decay)
        net = conv2d(net, 128, [1, 1], padding='VALID', stride=[1, 1],
                     bn=False, is_training=is_training, scope='conv2', bn_decay=bn_decay)
...
def up_projection_unit(inputs,up_ratio,scope="up_projection_unit",is_training=True,bn_decay=None):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        print('up_projection_unit inputs:', inputs.shape)
...
def coordinate_reconstruction_unit(inputs,scope="reconstruction",is_training=True,bn_decay=None):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        coord = conv2d(inputs, 64, [1, 1], padding='VALID', stride=[1, 1],
                       bn=False, is_training=is_training, scope='fc_layer1', bn_decay=None)

        coord = conv2d(coord, 3, [1, 1], padding='VALID', stride=[1, 1],
                       bn=False, is_training=is_training, scope='fc_layer2', bn_decay=None,
                       ctivation_fn=None, weight_decay=0.0)
        outputs = tf.squeeze(coord, [2])

        return outputs


def attention_unit(inputs, scope='attention_unit', is_training=True):   # b, n, k, c
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        dim = inputs.get_shape()[-1].value
        layer = dim//4
        f = conv2d(inputs, layer, [1, 1], padding='VALID', stride=[1, 1],
                   bn=False, is_training=is_training, scope='conv_f', bn_decay=None)    # b, n, k, layer
        g = conv2d(inputs, layer, [1, 1], padding='VALID', stride=[1, 1],
                   bn=False, is_training=is_training, scope='conv_g', bn_decay=None)
        h = conv2d(inputs, dim, [1, 1], padding='VALID', stride=[1, 1],
                   bn=False, is_training=is_training, scope='conv_h', bn_decay=None)
        s = tf.matmul(hw_flatten(g), hw_flatten(f), transpose_b=True)  # # [bs, N, N]
        beta = tf.nn.softmax(s, axis=-1)  # attention map
        o = tf.matmul(beta, hw_flatten(h))   # [bs, N, k, k]*[bs, N, k, c]->[bs, N, k, c]
...
def local_attention_unit(inputs, scope='attention_unit', is_training=True):   # b, n, k, c
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        h = conv2d(inputs, dim, [1, 1], padding='VALID', stride=[1, 1],
                   bn=False, is_training=is_training, scope='conv_h', bn_decay=None)
        s = tf.matmul(g, f, transpose_b=True)  # # b, n, k, k
        beta = tf.nn.softmax(s, axis=-1)  # attention map
        o = tf.matmul(beta, h)   # [bs, N, k, k]*[bs, N, k, c]->[bs, N, k, c]
        gamma = tf.get_variable("gamma", [1], initializer=tf.constant_initializer(0.0))
        o = tf.reshape(o, shape=inputs.shape)  # [bs, h, w, C]
        x = gamma * o + inputs
        x = tf.reduce_max(x, axis=2, keepdims=True, name='maxpool_1')  # b, N, c
    return x


def attention_unit_g(inputs, scope='attention_unit', is_training=True):   # b, n, k, c
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        dim = inputs.get_shape()[-1].value
        layer = dim//4
        f = conv2d(inputs, layer, [1, 1], padding='VALID', stride=[1, 1],
...
def conv1d(inputs,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if bn:
            outputs = tf.layers.batch_normalization(outputs, momentum=bn_decay, training=is_training, fused=True)
        if ibn:
...
def conv2d_wo_bn(input, n_cout, name, kernel_size=(1, 1),
...
def fully_connected(inputs,
    ...
    with tf.variable_scope(scope) as sc:
        ...
        if bn:
            outputs = tf.layers.batch_normalization(outputs, momentum=bn_decay, training=is_training)
...
def get_edge_feature(point_cloud, k=16, idx=None):
    ...
    # [N, P, K, Dim]
    print('point_cloud_neighbors idx', idx.shape)
    print('point_cloud_neighbors point_cloud', point_cloud.shape)
    point_cloud_neighbors = tf.gather_nd(point_cloud, idx)      # b, n, k, c
