...
def kernel_density_estimation_ball(pts, radius, sigma, N_points = 128, is_norm = False):
    with tf.variable_scope("ComputeDensity") as sc:
        ...
        g_pts = tf_grouping.group_point(pts, idx)   # b, n, N_points, 3
        g_pts -= tf.tile(tf.expand_dims(pts, 2), [1, 1, N_points, 1])   # - b, n, N_points, 3

        R = tf.sqrt(sigma)
        xRinv = tf.div(g_pts, R)
        quadform = tf.reduce_sum(tf.square(xRinv), axis = -1)   # b, n, N_points
        logsqrtdetSigma = tf.log(R) * 3
        mvnpdf = tf.exp(-0.5 * quadform - logsqrtdetSigma - 3 * tf.log(2 * 3.1415926) / 2)  # b, n, N_points

        first_val, _ = tf.split(mvnpdf, [1, N_points - 1], axis = 2)    # b, n, 1

        mvnpdf = tf.reduce_sum(mvnpdf, axis = 2, keepdims = True)       # b, n, 1

        num_val_to_sub = tf.expand_dims(tf.cast(tf.subtract(N_points, pts_cnt), dtype = tf.float32), axis = -1)

        val_to_sub = tf.multiply(first_val, num_val_to_sub)       # b, n, 1

        mvnpdf = tf.subtract(mvnpdf, val_to_sub)       # b, n, 1, distance to centroid term

        scale = tf.div(1.0, tf.expand_dims(tf.cast(pts_cnt, dtype = tf.float32), axis = -1))    # number of point term
...
def grouping(feature, K, src_xyz, q_xyz, use_xyz = True):
    ...
    point_indices = tf.py_func(knn_kdtree, [K, src_xyz, q_xyz], tf.int32)   # b, n, k
